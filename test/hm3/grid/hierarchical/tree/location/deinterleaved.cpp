...
int main() {
  {  // 1D (32 bit)
    test_location<1, 31>(location::deinterleaved<1, uint32_t>{});
  }

  {  // 2D (32 bit)
    test_location<2, 15>(location::deinterleaved<2, uint32_t>{});
  }

  {  // 3D (32_bit)
    test_location<3, 9>(location::deinterleaved<3, uint32_t>{});
  }
  {  // 1D (64 bit)
    test_location<1, 63>(location::deinterleaved<1, uint64_t>{});
  }

  {  // 2D (64 bit)
    test_location<2, 31>(location::deinterleaved<2, uint64_t>{});
  }

  {  // 3D (64_bit)
    test_location<3, 20>(location::deinterleaved<3, uint64_t>{});
  }
