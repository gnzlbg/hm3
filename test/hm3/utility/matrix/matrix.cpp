/// \file matrix.cpp Test matrix class
...
using test_types = boost::mpl::vector<num_t, idx_t, int_t, sidx_t, sint_t>;
...
template <template <class> class Lambda> void vector_test_all(const idx_t n) {
  using vectors =
   typename boost::mpl::transform<test_types,
                                  unit<Lambda<boost::mpl::_1>>>::type;

  boost::fusion::for_each(
   vectors{}, [n](auto v) { vector_test<typename decltype(v)::value>(n); });
}
