...
template <typename... Ts> constexpr auto make_array_(priority<0>, Ts&&...) {
  static_assert(false_t<Ts...>{},
                "The return types of all handlers must be equivalent for "
                "return type deduction of `type_switch`.");
}

template <typename... Ts> constexpr decltype(auto) make_array(Ts&&... ts) {
  return make_array_(priority<1>{}, forward<Ts>(ts)...);
}

/* `at` */

template <typename T> constexpr const T& at_(const T& elem, const size_t*) {
  return elem;
}
...
template <typename... Vs> struct vtable {
  ...
  static constexpr auto make(index_sequence<Is...>, index_sequence<Js...>,
                             Ls... ls) {
    return make_array(make<F>(index_sequence<Is..., Js>{}, ls...)...);
  }
};  // vtable

template <typename F, typename... Vs> constexpr auto make_vtable() {
