...
template <typename... Ts> class variant_base {
 ...
 protected:
  ...
  template <size_t I, typename... Args> void construct(Args&&... args) {
...
class variant_impl_<meta::list<Ts...>,
 ...
 protected:
  ...
  template <size_t I, typename Arg> void assign(Arg&& arg) {
  ...
  void swap(variant_impl_& that) noexcept(
   noexcept(std::swap(declval<variant_impl_&>(), declval<variant_impl_&>()))) {
    std::swap(*this, that);
  }
};
...
class variant_impl_<meta::list<Ts...>,
 ...
 protected:
  ...
  template <size_t I, typename Arg> void assign(Arg&& arg) {
    ...
    if (this->index() == I) {
      ...
      struct emplacer {
        void operator()(const T& arg_) const { self.emplace<I>(T(arg_)); }
        void operator()(T&& arg_) const { self.emplace<I>(move(arg_)); }
        variant_impl_& self;
      };  // emplacer
 ...
 private:
  template <size_t I> struct assigner {
  ...
  template <size_t I> struct constructor {
    template <typename Arg> void operator()(Arg&& arg) const {
  ...
  struct swapper {
    template <typename T> void operator()(T& lhs, T& rhs) const {
      using std::swap;
      swap(lhs, rhs);
    }

    template <typename T, typename U> void operator()(T&, U&) const {
      assert(false);
    }
  };  // swapper
};
...
class variant_impl_<meta::list<Ts...>,
 ...
 protected:
  ...
  variant_impl_& operator=(const variant_impl_& that) {
    if (this->index() != that.index()) { destroy(); }  // if
    super::operator=(that);
    return *this;
  }
  ...
  void swap(variant_impl_& that) noexcept(
   noexcept(declval<variant_impl_&>().super::swap(declval<variant_impl_&>()))) {
    super::swap(that);
  }
 ...
 private:
  ...
  template <size_t I> struct destructor {
    template <typename Arg> void operator()(Arg&) const noexcept {
...
template <typename... Ts>
using variant_impl
 = variant_impl_<meta::list<Ts...>, meta::and_<is_trivially_copyable<Ts>...>{},
                 meta::and_<is_trivially_destructible<Ts>...>{}>;

template <typename... Ts> class variant : public variant_impl<Ts...> {
  using alternatives = meta::list<Ts...>;
 ...
 private:
  ...
  template <typename T, typename Arg> void assign(Arg&& arg) {
    static_assert(meta::count<alternatives, T>{} == 1, "");
...
template <typename... Ts> class variant : public detail::variant<Ts...> {
  using alternatives = meta::list<Ts...>;
 ...
 public:
  ...
  explicit constexpr variant(in_place_index_t<I> ip, initializer_list<U> init,
                             Args&&... args)
  ...
  explicit constexpr variant(in_place_type_t<T> ip, initializer_list<U> init,
                             Args&&... args)
   : super(ip, init, forward<Args>(args)...) {}
  ...
  template <size_t I, typename U, typename... Args>
  meta::if_<is_constructible<meta::at_c<alternatives, I>, initializer_list<U>&,
                             Args&&...>,
            void>
   emplace(initializer_list<U> init, Args&&... args) {
    super::template emplace<I>(init, forward<Args>(args)...);
  }

  template <typename T, typename U, typename... Args>
  meta::if_<is_constructible<T, initializer_list<U>&, Args&&...>, void> emplace(
