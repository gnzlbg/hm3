...
constexpr bool sd_intersection_test(SDValueRng&& rng) noexcept {
  auto it = begin(rng);
  auto e  = end(rng);
...
constexpr bool sd_intersection_test(SDValueRng&& rng) noexcept {
  return sd_signum_intersection_test(
   rng | view::transform([](auto&& v) { return math::signum(v); })) false;
}
...
static constexpr bool sd_intersection_test(P&& p, SD&& sdf) noexcept {
