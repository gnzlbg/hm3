...
struct parallel_line_fn {
  ...
  static constexpr auto impl(T const&, T const&, num_t, num_t, fallback)
  ...
  constexpr auto operator()(T const& a, T const& b, num_t abs_tol,
                            num_t rel_tol) const
   RANGES_DECLTYPE_AUTO_RETURN_NOEXCEPT(impl(a, b, abs_tol, rel_tol, dispatch));
};

}  // namespace parallel_line_detail

namespace {
constexpr auto const& parallel_line = static_const<
