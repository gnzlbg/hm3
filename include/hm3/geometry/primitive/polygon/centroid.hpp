...
constexpr auto centroid_2d(P&& p) noexcept -> point<dimension_t<P>{}> {
  ...
  if (!vertex_order.counter_clockwise(p)) { x() *= -1.; }
  return x;
}
...
constexpr auto centroid(P&& p_) noexcept -> point<dimension_v<P>> {
#if !defined(HM3_GEOMETRY_POLYGON_USE_GENERIC_CENTROID)
  if (vertex_size(p_) == 3) {
    return centroid_detail::centroid_triangle(std::forward<P>(p_));
  }
#endif
  // transforms polygon into planar coordinates:
  auto b  = planar.basis(p_);
  auto pp = planar.to(p_, b);
