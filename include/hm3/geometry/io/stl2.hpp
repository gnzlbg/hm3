...
struct reserve_fn {
  ...
  template <typename Mesh>
  constexpr auto /* bool */ impl(Mesh&& mesh, std::uint32_t n,
                                 hm3::preferred) const
  ...
  template <typename Mesh>
  constexpr auto /* bool */ impl(Mesh&&, std::uint32_t, hm3::fallback) const
   -> bool {
    return true;
  }
  ...
  constexpr auto /* bool */ operator()(Mesh&& mesh, std::uint32_t n) const
   -> decltype(impl(std::forward<Mesh>(mesh), n, hm3::dispatch), bool()) {
    return impl(std::forward<Mesh>(mesh), n, hm3::dispatch);
  }
};
