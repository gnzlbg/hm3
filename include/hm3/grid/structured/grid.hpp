...
#include <hm3/grid/structured/cell.hpp>
...
struct grid : geometry::dimensional<Nd> {
  ...
  constexpr void cell_length(num_t new_cell_length) noexcept {
  ...
  constexpr coordinate_t internal_cell_containing(point_t x) const noexcept {
    ...
    for (suint_t d = 0; d < Nd; ++d) {
      ...
      HM3_ASSERT(i[d] >= cell_indices::first_internal()
                 ...
                 "cell containing point {} lies outside the internal cells for "
    ...
    HM3_ASSERT(c.is_internal(),
               "internal cell {} containing point {} is not internal?", c, x);
    return c;
  }
