...
/// \file
///
/// Serialization of grid::hierarchical::cartesian::single to VKT
...
/// Adapts a single<Ad> to be Serializable to a vtkUnstructuredGrid
///  - a cell_geometry(node_idx) method
///  - a bounding_box() method
///  - a dimensions() method
///  - a for_each_cell(F&&) method
template <dim_t Ad>
struct serializable_single : geometry::with_ambient_dimension<Ad> {
  single<Ad> const& t_;
  sint_t level_ = -1;

  using vtk_cell_idx = tree_node_idx;

  serializable_single(single<Ad> const& s, sint_t level = -1) noexcept
   : t_{s}, level_{level} {}
...
template <dim_t Ad>
void serialize(single<Ad> const& t, string const& fname_, sint_t level = -1) {
  using serializable = serializable_single<Ad>;
  hm3::log::serial log("single-tree<Ad>-vtk-dump");
