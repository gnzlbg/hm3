#pragma once
/// \file multi.hpp Multiple Solver Grids
...
struct multi : TreeGrid {
  ...
  /// Number of grids
  auto no_grids() const HM3_RETURNS(grid_idx{grids_().cols()});

  /// Range of all grid ids.
  inline auto grids() const HM3_RETURNS(boxed_ints<grid_idx>(0_g, no_grids()));
