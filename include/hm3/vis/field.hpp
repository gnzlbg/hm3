#pragma once
/// \file field.hpp Visualization field
#include <string>
#include <functional>
...
struct field {
  ...
  field(string const& n, std::function<void(void)> f, const int v = 0)
   : name{n}, value{static_cast<value_t>(v)}, load{std::move(f)} {}
  field(string const& n, std::function<void(void)> f, const value_t v)
   : name{n}, value{v}, load{std::move(f)} {}
...
template <class C, class UC = uncvref_t<C>,
