...
struct unstructured_grid {
  ...
  ///   geometry of a cell
  template <class NodeRange, class Grid,
  ...
  void append_internal_cells(NodeRange&& nodes, Grid&& grid) noexcept {
    ...
    const auto no_cells = distance(use_(nodes));
    ...
    for (auto d : grid.dimensions()) {
      bounds_vtk[d]     = bounds[0](d);
      bounds_vtk[d + 3] = bounds[1](d);
    }
  ...
  template <class Nodes, class Grid>
