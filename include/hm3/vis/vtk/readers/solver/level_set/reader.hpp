#pragma once
/// \file reader.hpp Level-set reader
...
template <uint_t nd> struct reader : ::hm3::vis::vtk::reader {
 ...
 private:
  ...
  void register_cell_data() noexcept {
    ...
    if (!cell_data.assert_unique()) { HM3_TERMINATE("fields are not unique!"); }
  }
...
inline auto make(io::json const& b)
 -> std::unique_ptr<::hm3::vis::vtk::reader> {
  ...
  if (block_type != "level_set") {
    HM3_TERMINATE("Cannot make a level_set reader from a block of type \"{}\"",
                  block_type);
  }
