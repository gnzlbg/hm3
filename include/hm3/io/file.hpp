#pragma once
/// \file file.hpp Describes a single file
...
template <class T> bool correct_datatype(T, string const& s) {
...
struct file {
 ...
 private:
  ...
  hm3::log::serial log = string{"file_log"};
 ...
 public:
  ...
  template <class T> file& field(string const& field_name, T value) {
  ...
  file& field(string const& field_name, T* begin, T* end,
    ...
    auto p = std::pair<void*, void*>((void*)begin, (void*)end);
  ...
  explicit file(io::json header) {
    ...
    if (!valid_header) { HM3_TERMINATE("Invalid file header:\n\n{}", header); }
 ...
 public:
  void write_data_only(mpi::comm const&) const {
    log("writing file \"{}\" arrays...", path());
    log("- with header:\n\n{}\n", header().dump(2));
    log("- field | mem: [from, to) | size:");
    std::ofstream o(path());
    using byte_ptr = char const* const;
    for (auto&& f : in_memory_) {
      ...
      log("    {} | mem: [{}, {}) : {}", f.first, (void*)mb, (void*)me, m_size);
      o.write(mb, m_size);
    }
    log("...writing file \"{}\" arrays done!", path());
  }
  ...
  void read_arrays() {
    log("reading file \"{}\" arrays...", path());
    log("- with header:\n\n{}\n", header().dump(2));
    log("- field | file [from, to) : size | mem: [from, to) : size:");
    std::ifstream fh(path());
    if (fh.fail()) { HM3_TERMINATE("couldn't open file: \"{}\"", path()); }
    using byte_ptr = char*;
    for (auto i = begin(fields_), e = end(fields_); i != e; ++i) {
      ...
      if (in_memory_.count(n)) {
        ...
        fh.read(mb, m_size);
        log("    {} | file [{}, {}) : {} | mem [{}, {}) : {} | succeded: {}", n,
            fb, fe, f_size, (void*)mb, (void*)me, m_size,
            fh.fail() ? "false" : "true");
      }
    }
    log("...reading file\"{}\" arrays done!", path());
  }
};
