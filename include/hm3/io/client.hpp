#pragma once
/// \file client.hpp Helper class to interact with an I/O session
...
struct client {
 ...
 public:
  ...
  client(io::session& s, string const& name) : session_(&s), name_(name) {
    if (!session().has(name_)) {
      HM3_TERMINATE("Could not find client \"{}\" in session \"{}\"", name_,
                    s.file_path());
    }
  ...
  client(io::session& s, string const& name, string const& type,
   ...
   : session_(&s) {
    ...
    if (!session().has(name)) {
      ...
      return;
    } else {  // Group already registered: we must be reading from a session
      // read the session:
      (*this) = io::client(s, name);

      // verify arguments:
      if (type_ != type) {
        HM3_TERMINATE("mismatching file group types for client:\n\n  name: "
                      "{}\n  type: {}\n  "
                      "dependency: {}\n\n  passed type: \"{}\"\n\n",
  ...
  io::session& session() {
    if (!session_) {
      HM3_TERMINATE("io::client for \"{}\" is not initialized (null session)",
                    name_);
    }
    return *session_;
  }
