#pragma once
/// \file filesystem.hpp File system utilities.
...
HM3_REQUIRES(std::is_convertible<system::tag_t, system::posix>{})
...
inline string read_whole(string const& file_name, const mpi::comm& comm) {
  if (!exists(file_name, comm)) {
    HM3_TERMINATE("file name \"{}\" does not exist!", file_name);
  }
