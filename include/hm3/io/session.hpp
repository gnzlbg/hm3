...
struct session {
 ...
 public:
  ...
  static bool remove(string path, mpi::comm const& c) {
    ...
    return fs::remove(path.c_str(), c);
  }

  session(mpi::comm const& comm) : comm_{comm} {}
  ...
  session(create_t, string const& path, mpi::comm const& comm) : comm_{comm} {
    set_file(create, path);
  }

  /// Restart from an already existing session
  session(restart_t, string const& path, mpi::comm const& comm) : comm_{comm} {
 ...
 private:
  ...
  void set_file(restart_t, string path) {
    set_path_extension(path);
    if (!fs::exists(path, comm_)) {
      HM3_TERMINATE("Cannot restart session {}, file: \"{}\" not found!", name_,
                    path);
    }
