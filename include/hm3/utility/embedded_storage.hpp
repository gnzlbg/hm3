...
/// Forces embedded_storage to align its contents at list to the SIMD alignment boundary.
...
struct empty_storage {
  ...
  static constexpr void unsafe_set_size(size_type new_size) noexcept {
    HM3_ASSERT(new_size == 0, "tried to change size of empty storage to {}", new_size);
  }
...
struct trivial_storage {
 ...
 public:
  ...
  constexpr trivial_storage& operator=(trivial_storage const&) noexcept = default;
...
private:
  ...
  template <typename U, CONCEPT_REQUIRES_(ConvertibleTo<U, T>{})>
  static constexpr data_t unsafe_recast_init_list(std::initializer_list<U>& il) noexcept {
...
struct non_trivial_storage {
 ...
 private:
  ...
  using data_t = meta::if_c<not std::is_const<T>{},
                            aligned_storage_t, const aligned_storage_t>;
 ...
 public:
  ...
  ///
  /// Complexity: O(1) in time and space.
  constexpr const T* data() const noexcept { return reinterpret_cast<const T*>(data_); }
  ...
  noexcept(noexcept(new (data() + size()) T(std::forward<Args>(args)...))) {
    HM3_ASSERT(not full(), "tried to emplace_back on full storage");
  ...
  /// \warning: The size of the storage is not changed.
  template <typename InputIt, CONCEPT_REQUIRES_(InputIterator<InputIt>{})>
  constexpr void unsafe_destroy(InputIt first, InputIt last)
    noexcept(std::is_nothrow_destructible<T>{}) {
  ...
  ///
  /// \warning: The size of the storage is not changed.
  constexpr void unsafe_destroy_all() noexcept(std::is_nothrow_destructible<T>{}) {
  ...
  constexpr non_trivial_storage& operator=(non_trivial_storage const&) = default;
  ...
  /// Contract: `il.size() <= capacity()`.
  template <typename U, CONCEPT_REQUIRES_(ConvertibleTo<U, T>{})>
  constexpr non_trivial_storage(std::initializer_list<U> il)
    noexcept(noexcept(emplace_back(il.begin()[0]))) {
    HM3_ASSERT(il.size() <= capacity(),
               "trying to construct storage from an initializer_list whose "
               "size (= {}) exceed the storage capacity (= {}})",
               il.size(), capacity());
