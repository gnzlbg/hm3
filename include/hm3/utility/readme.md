...
```c++
HM3_TERMINATE(message); // exits the program
```
...
it so hard errors (using `HM3_TERMINATE`) have been the preferred way of dealing
...
- **Stack allocation**:
  ...
  - stack allocator for containers:
    ...
    ```
  - **Note**: `HM3_TERMINATE` is called if you run out of memory.
