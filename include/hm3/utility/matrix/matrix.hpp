#pragma once
/// \file matrix.hpp Dense matrix implementation
...
template <class T,                       /// value type
          ...
          class RowIdx  = idx_t,         /// row index
          class ColIdx  = idx_t,         /// column index
          class Order   = col_major_t,   /// memory layout (col/row-major-order)
...
struct matrix : bounds<NoRows, NoCols>,
  ...
  template <class I, class U = std::remove_reference_t<std::remove_cv_t<I>>,
            HM3_REQUIRES_(
             is_vector()
             && std::is_same<typename std::iterator_traits<U>::value_type,
                             T>{})>
  ...
  template <class Other> void element_wise_assign(Other&& o) {
  ...
  template <typename Expr, int Rows, int Cols, bool I, HM3_REQUIRES_(!is_bit())>
  ...
  /// Construct from Eigen expression:
  HM3_REQUIRES(!is_bit())  // this can be relaxed
 ...
 public:
  ...
  auto begin() noexcept { return std::begin(this->data_); }
  auto begin() const noexcept { return std::begin(this->data_); }
  auto end() noexcept { return std::begin(this->data_) + size(); }
  auto end() const noexcept { return std::begin(this->data_) + size(); }
};

template <class T, int_t NoRows, int_t NoCols, class RowIdx = idx_t,
          class ColIdx = idx_t, class Order = col_major_t,
          int_t MaxRows = NoRows, int_t MaxCols = NoCols,
          template <class> class Vector,
          class M = matrix<T, NoRows, NoCols, RowIdx, ColIdx, Order, MaxRows,
                           MaxCols, Vector>,
          HM3_REQUIRES_(M::is_vector() && !M::is_bit())>
constexpr auto begin(
 matrix<T, NoRows, NoCols, RowIdx, ColIdx, Order, MaxRows, MaxCols, Vector>& m)
 HM3_RETURNS(std::begin(m.data_));
...
constexpr auto end(
 matrix<T, NoRows, NoCols, RowIdx, ColIdx, Order, MaxRows, MaxCols, Vector>& m)
 HM3_RETURNS(std::begin(m.data_) + m.size());
...
constexpr auto cend(matrix<T, NoRows, NoCols, RowIdx, ColIdx, Order, MaxRows,
                           MaxCols, Vector> const& m)
 HM3_RETURNS(m.data_.cbegin() + m.size());

template <class T, int_t NoRows, class RowIdx = idx_t, int_t MaxRows = NoRows,
