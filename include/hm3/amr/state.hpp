#pragma once
/// \file state.hpp Adaptive mesh refinement state
#include <vector>
...
template <class Target> struct state {
 private:
  /// Node idx type
 ...
 public:
  ...
  template <class Action> void compute_actions(Action&& action) {
  ...
  template <class AfterIt = std::function<void()>>
  ...
  template <class Action, class AfterIt = std::function<void()>>
