#pragma once
/// \file level_at_distance.hpp Cells at a given distance from the boundary
/// are refined until they are at a given level
#include <vector>
#include <utility>
#include <type_traits>
...
struct level_till_distance {
  ...
  action operator()(Grid&& grid, SD&& distance_fn, Node n) const {
    ...
    if (node_distance < target_distance) {
      ...
      } else if (node_level > target_level) {
        return action::coarsen;
      } else {  // node_level == target_level:
        return action::none;
      }
    } else {  // node_distance >= target_distance
      return action::coarsen;
    }
  }
};
...
struct level_till_cell_distances {
 private:
  ...
  multiple<level_till_distance> r;
 ...
 public:
  ...
  level_till_cell_distances(Grid const& grid, std::initializer_list<step> l)
   : r(l.size()) {
    ...
    num_t distance = 0.;
    transform(steps_, begin(r.steps), [&](auto&& s) {
  ...
  amr::action operator()(Grid&& grid, SD&& sd, Node n) const {
    return r(std::forward<Grid>(grid), std::forward<SD>(sd), n);
  }
};
