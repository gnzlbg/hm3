...
// In the paraview plugin the grid doesn't need to load the grid ids
 ...
 public:
  ...
  void swap(grid_node_idx i, grid_node_idx j) {
    ...
    // HM3_ASSERT(old_gn_i == new_gn_j, "old i {}, old j {}, new i {}, new j
    // {}",
    //            old_gn_i, old_gn_j, new_gn_i, new_gn_j);

    // HM3_ASSERT(old_tn_j == tree_node_ids_(i), "");
    // HM3_ASSERT(old_if_j == is_free_(i), "");
    // HM3_ASSERT(old_gn_j == new_gn_i, "old i {}, old j {}, new i {}, new j
    // {}",
    //            old_gn_i, old_gn_j, new_gn_i, new_gn_j);
  }
 ...
 public:
  ...
  struct refine_t {
    ...
    refine_t(grid& g, grid_node_idx n) : grid_{g}, parent_{std::move(n)} {}
  ...
  struct coarsen_t {
    grid& grid_;
    grid_node_idx parent_;
    coarsen_t(grid& g, grid_node_idx n) : grid_{g}, parent_{std::move(n)} {}
  ...
  grid(tree_t& t, grid_idx g, grid_node_idx node_capacity,
       hm3::log::serial& log)
   : grid(t, std::move(g), std::move(node_capacity)) {
