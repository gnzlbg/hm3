#pragma once
/// \file state.hpp Level-set solver state
...
template <uint_t nd_> struct state {
  ...
  cell_idx push(const node_idx n) noexcept { return g.push(n); };

  /// Removes the solver cell \p c from the solver grid
  void pop(const cell_idx i) noexcept { g.pop(i); }
