...
#include <hm3/solver/geometry/boundary_surface.hpp>
  ...
  if
    ...
    constexpr(Ad == 3) {
      const num_t azimuthal_coordinate
       = random_float(random_gen, num_t{0}, 2. * math::pi);
      const num_t polar_coordinate
       = random_float(random_gen, num_t{0}, math::pi);
      return vec_t{
...
template <dim_t Ad>
struct discrete_boundary : hg::with_ambient_dimension<Ad> {
  ...
  using simplex_t = typename surface_t::simplex_t;
 ...
 public:
  ...
  uint_t edge_size() const noexcept {
    if
      constexpr(Ad == 1) { return 0; }
    else if
      constexpr(Ad == 2) { return vertex_size(); }
    else if
      constexpr(Ad == 3) { return edge_surfaces_.size(); }
  }
 ...
 private:
  ...
  template <typename At>
  static void assert_valid(bidx_t v, At&& at) noexcept {
 ...
 public:
  ...
  auto const& edge_surfaces_ids(eidx_t e) const noexcept {
    if
      constexpr(Ad == 1) { return; }
    else if
      constexpr(Ad == 2) { return vertex_surfaces_ids(e); }
    else if
      constexpr(Ad == 3) {
        assert_valid(e, HM3_AT_);
        HM3_ASSERT(*e < edge_surfaces_.size(),
                   "edge {} is out-of-bounds [0, {})", e,
                   edge_surfaces_.size());
 ...
 public:
  ...
  auto const& surface_edges_ids(sidx_t s) const noexcept {
    if
      ...
      constexpr(Ad == 2) { return surface_vertices_ids(s); }
    else if
      constexpr(Ad == 3) {
        assert_valid(s, HM3_AT_);
  ...
  auto& surface_edge_id(sidx_t s, suint_t e) noexcept {
    if
      ...
      constexpr(Ad == 2) { return surface_vertex_id(s, e); }
    else if
      constexpr(Ad == 3) {
  ...
  edge_t edge(eidx_t e) const noexcept {
    if
      ...
      constexpr(Ad == 2) { return edge_t{vertex(*e)}; }
    else if
      constexpr(Ad == 3) {
 ...
 private:
  void surface_data_append(sidx_t s, bidx_t d) {
    assert_valid(s, HM3_AT_);
    ...
    if
      constexpr(Ad == 3) {
        ...
        auto edge_surface_append = [&](auto&& e, auto&& s) {
          ...
          HM3_ASSERT(not es.full(), "edge->surface adjacency of edge {} full",
                     e);
          es.push_back(s);
        };
        ...
        for (auto e : edges) {
          ...
          if (s) {
            ...
            HM3_ASSERT(
             ...
             make_pair(
              surface_vertex_id(s, local_eidx == 2 ? 0 : local_eidx + 1),
              vertex(
               surface_vertex_id(s, local_eidx == 2 ? 0 : local_eidx + 1))));
 ...
 public:
  ...
  discrete_boundary()                         = default;
  discrete_boundary(discrete_boundary const&) = default;
  discrete_boundary(discrete_boundary&&)      = default;
  discrete_boundary& operator=(discrete_boundary const&) = default;
  discrete_boundary& operator=(discrete_boundary&&) = default;
  ...
  template <
   ...
   CONCEPT_REQUIRES_(
    Range<SurfaceRng>{}
    and Same<uncvref_t<ranges::range_value_t<SurfaceRng>>, surface_t>{})>
  discrete_boundary(SurfaceRng&& surface_rng) {
    /// Insert the surfaces:
...
void serialize(State const& state, string file_name) {
  hm3::log::serial log("solver::discrete_boundary-serialization-to-vtk");
