...
struct bcs {
 ...
 private:
  ...
  using boundary_cells = vector<cell_idx_t>;
  vector<boundary_cells> cells_in_bc_;

  boundary_cells& cells_in_bc(boundary_idx_t i) noexcept {
    HM3_ASSERT(i < boundary_idx_t(size()),
               "boundary condition index {} is out-of-bounds [0, {})", i,
               size());
    return cells_in_bc_[*i];
  }

  boundary_cells const& cells_in_bc(boundary_idx_t i) const noexcept {
    HM3_ASSERT(i < boundary_idx_t(size()),
               "boundary condition index {} is out-of-bounds [0, {})", i,
               size());
    return cells_in_bc_[*i];
 ...
 public:
  ...
  boundary_idx_t push() noexcept {
    boundary_idx_t i = size();
  ...
  void clear(boundary_idx_t i) { cells_in_bc(i).clear(); }
 ...
 private:
  ...
  void for_each_bc(F&& f) noexcept {
    for (boundary_idx_t i = 0; i != size(); ++i) { f(i); }
  }
  template <typename F>
 ...
 public:
  ...
  bool has_cell(cell_idx_t cidx, boundary_idx_t bcidx) const noexcept {
  ...
  fixed_capacity_vector<boundary_idx_t, max_bcs_per_cell> boundary_condition(
   cell_idx_t cidx) {
    fixed_capacity_vector<boundary_idx_t, max_bcs_per_cell> bc_ids;
    for_each_bc([&](boundary_idx_t i) {
 ...
 private:
  ...
  void push(cell_idx_t cidx, boundary_idx_t bcidx) {
  ...
  void remove(boundary_idx_t i, UnaryPredicate&& up) {
  ...
  void remove(BinaryPredicate&& bp) {
    for_each_bc([&](boundary_idx_t i) {
      remove(i, [&](cell_idx_t j) { return bp(j, i); });
    });
  }
  ...
  struct inserter : modifier {
    ...
    void insert(cell_idx_t c, boundary_idx_t b) { s_.push(c, b); }
  };
  ...
  struct remover : modifier {
    ...
    using modifier::operator=;
    template <typename UnaryPredicate>
    void remove(boundary_idx_t b, UnaryPredicate&& up) {
