#pragma once
////////////////////////////////////////////////////////////////////////////////
// \file Includes unit/quantity types
////////////////////////////////////////////////////////////////////////////////
// Includes:
...
using dimensionless = NumQ<busi::dimensionless>;              ///< [-]
using length = NumQ<busi::length>;                            ///< [m]
using area = NumQ<busi::area>;                                ///< [m2]
using volume = NumQ<busi::volume>;                            ///< [m3]
using energy = NumQ<busi::energy>;                            ///< [J]
using time = NumQ<busi::time>;                                ///< [s]
using mass = NumQ<busi::mass>;                                ///< [kg]
using temperature = NumQ<busi::temperature>;                  ///< [K]
using density = NumQ<busi::mass_density>;                     ///< [kg/m3]
using velocity = NumQ<busi::velocity>;                        ///< [m/s]
using pressure = NumQ<busi::pressure>;                        ///< [kg/(ms2)]
using energy_density = NumQ<busi::energy_density>;            ///< [J/m3]
using specific_energy = NumQ<busi::specific_energy>;          ///< [J/kg]
using dynamic_viscosity = NumQ<busi::dynamic_viscosity>;      ///< [kg/(m s)]
using thermal_diffusivity = NumQ<busi::thermal_diffusivity>;  ///< [m^2/s]
using specific_gas_constant
 = NumQ<busi::specific_gas_constant>;  ///< [J/(kg K)]
...
/// Memory units
using byte
 = boost::units::quantity<boost::units::byte_base_unit::unit_type, Num>;
using mega_byte
 = boost::units::quantity<boost::units::megabyte_base_unit::unit_type, Num>;
using giga_byte
 = boost::units::quantity<boost::units::gigabyte_base_unit::unit_type, Num>;
...
template <class T> unit::mega_byte amount(T* from, T* to) {
  char* from_c = reinterpret_cast<char*>(from);
  char* to_c = reinterpret_cast<char*>(to);
  return unit::mega_byte{Num{to_c - from_c} * unit::bytes};
}

template <class T> unit::mega_byte amount(T* from, Ind size) {
  T* to = from + size;
  return amount(from, to);
}

template <class Container> unit::mega_byte amount(Container&& c) {
